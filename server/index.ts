...
const validateIncidentType = (type: string): type is 'medical' | 'security' | 'facility' | 'lost-person' => {
  return ['medical', 'security', 'facility', 'lost-person'].includes(type);
};

const validatePriority = (priority: string): priority is 'low' | 'medium' | 'high' | 'critical' => {
  return ['low', 'medium', 'high', 'critical'].includes(priority);
};

// Enhanced input validation middleware
const validateIncidentInput = (req: express.Request, res: express.Response, next: express.NextFunction) => {
...
app.post('/api/incidents', strictRateLimiter, validateIncidentInput, (req, res) => {
  ...
  const type = req.body.type as 'medical' | 'security' | 'facility' | 'lost-person';
  const location = sanitizeStr(req.body.location, 80);
  const priority = req.body.priority as 'low' | 'medium' | 'high' | 'critical';
  const reportedBy = sanitizeStr(req.body.reportedBy, 100);

  try {
    const newIncident = addIncident({ title, description, type, location, priority, reportedBy });
