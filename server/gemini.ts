...
const simulateResponse = (prompt: string, role: string): string => {
  const query = prompt.toLowerCase();
  
  if (role === 'admin' || query.includes('summarize current') || query.includes('stadium condition') || query.includes('operations')) {
    const stats = getDashboardStats();
    const activeIncidents = getIncidents().filter(i => i.status !== 'resolved');
...
export const generateGeminiResponse = async (prompt: string, role: string = 'fan'): Promise<string> => {
  if (!genAI) {
  ...
  try {
    ...
    if (role === 'admin' && (prompt.toLowerCase().includes('summarize') || prompt.toLowerCase().includes('status') || prompt.toLowerCase().includes('operations'))) {
      const stats = getDashboardStats();
      const openIncidents = getIncidents().filter(i => i.status !== 'resolved');
